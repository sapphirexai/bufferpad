...
| 设置 / 设备信息 | 维护扫码器、三菱 PLC、汇川 PLC、西门子 S7 PLC 的连接参数 |
| 设置 / PLC地址 | 维护扫码器操作类型与 PLC 寄存器的映射 |

选择西门子 S7 PLC 时，设备端口自动建议为 `102`；该类型同时适用于 S7-1200 和 S7-1500，PLC 地址页会提示使用 `DB1.DBW0`、`MW0`、`IW0`、`QW0` 等 16 位绝对地址。
...
- 最近一次扫码或 PLC 处理结果使用单行反馈条展示，长文本自动省略；现场处理建议、二维码、
  扫码器名称/IP、PLC 名称/IP、寄存器地址和错误码统一放入“运行告警与最近操作”抽屉，正文不会再压住下方内容。
...
`/api/operationEvents/recent` 确认事件接口已返回扫码器/PLC 名称与 IP 字段。
