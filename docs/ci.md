...
PowerShell CI 使用 `pwsh`（PowerShell 7）。语法检查器显式按 UTF-8 读取源码，在 Windows PowerShell 5.1 和 PowerShell 7 下均可执行；这不代表安装器已在两个运行时中完成正式安装验收。Windows PowerShell 5.1 直接执行部分无 BOM 中文脚本可能产生本地编码问题，现场部署前需单独验证；当前脚本验证基线为 PowerShell 7。
